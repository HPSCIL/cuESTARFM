...
void runtest_pairs(CuLayer *psensor,PARAMETER *par,int solve)
{
	...
	//cudaSetDevice(0);
	cudaMemGetInfo(&ff, &tt);
	maxnum=(ff-sizeof(float)*par->WIN_SIZE*par->WIN_SIZE*num_block*num_thread*2)/(psensor[0].getbandCount()*sizeof(float)*2*(par->NUM_PAIRS+1));
...
void Re_fusion3(CuLayer *psensor,PARAMETER *par)
{
	...
	for(c=0;c<par->NUM_PREDICTIONS;c++)
	{
		...
		if(par->NUM_PAIRS==2)
		{
			...
			psensor[2*(par->NUM_PAIRS+c)+1].Write(psensor[2*(par->NUM_PAIRS+c)+1].outpath);
		}
		else
		{
