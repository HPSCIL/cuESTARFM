...
using namespace std;
	...
	public:
		CuLayer();
		CuLayer(float* data, int width, int height,int count);
		CuLayer(int width, int height,int count);
		~CuLayer();
		CuLayer(const CuLayer &culayer);
		int Read(string filePath);
		int Write(string filePath,string Type);
		void resize(int width, int height);
		void resize(int width, int height,int bandcount);
		void resize(float* data, int width, int height);
		const int getbandCount()const{ return this->bandCount; };
